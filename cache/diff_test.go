...
func TestMergeIdRefs(t *testing.T) {
	...
	bunch = mergeBunch(bunch, []element.IdRefs{{50, []int64{1}}})
	if b := bunch[0]; b.Id != 50 || b.Refs[0] != 1 {
		t.Fatal(bunch)
	}

	// before
	bunch = mergeBunch(bunch, []element.IdRefs{{40, []int64{3}}})
	if b := bunch[0]; b.Id != 40 || b.Refs[0] != 3 {
		t.Fatal(bunch)
	}

	// after
	bunch = mergeBunch(bunch, []element.IdRefs{{70, []int64{4}}})
	if b := bunch[2]; b.Id != 70 || b.Refs[0] != 4 {
		t.Fatal(bunch)
	}

	// in between
	bunch = mergeBunch(bunch, []element.IdRefs{{60, []int64{5}}})
	if b := bunch[2]; b.Id != 60 || b.Refs[0] != 5 {
		t.Fatal(bunch)
	}

	// same (50:1 already inserted)
	bunch = mergeBunch(bunch, []element.IdRefs{{50, []int64{0, 5}}})
	...
	// remove multiple
	bunch = mergeBunch(bunch, []element.IdRefs{{40, []int64{}}, {60, []int64{}}})
	if bunch[0].Id != 50 || bunch[1].Id != 70 || len(bunch) != 2 {
		t.Fatal(bunch)
	}

	// add multiple
	bunch = mergeBunch(bunch, []element.IdRefs{{40, []int64{1}}, {60, []int64{1}}, {80, []int64{1}}})
